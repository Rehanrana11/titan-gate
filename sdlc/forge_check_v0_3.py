#!/usr/bin/env python3
"""forge_check v0.3 -- lint pasteable blocks, and emit the output contract.

CHANGELOG
    v0.3  outbound parser rewritten. v0.2 skipped rows CONTAINING "RETRACTED",
          but a retraction row retracts the row ABOVE it -- so v0.2 counted two
          retracted rows as valid and printed "2/0" against a log with zero
          valid rows. Found on first run against real data. A plausible wrong
          number from an anti-fabrication tool is the worst class of output this
          project has a name for. Also: a row with an empty sent cell is not a
          count of zero, it is not a count.
...
def _outbound(path):
    """Count VALID rows in the outbound log.

    Retraction semantics, learned the hard way: a row whose channel is
    RETRACTED retracts the DATA ROW ABOVE IT. v0.2 skipped the retraction row
    and counted its target, which inverted the whole convention.

    A row with an empty `sent` cell is not zero. It is not a count at all.
    """
    ...
    for line in open(path, encoding="utf-8", errors="replace"):
        ...
        if len(cells) < 2 or cells[0].lower() in ("date", "") or set(cells[0]) <= set("-: "):
            continue
        rows.append(cells)

    retracted = set()
    for i, cells in enumerate(rows):
        if cells[1].upper() == "RETRACTED":
            retracted.add(i)                       # the retraction row itself
            if i > 0:
                retracted.add(i - 1)               # and its target, the row above
    ...
    for i, cells in enumerate(rows):
        if i in retracted:
            continue
        s_cell = cells[3] if len(cells) > 3 else ""
        r_cell = cells[4] if len(cells) > 4 else ""
        if not s_cell.strip() or not s_cell.strip().lstrip("-").isdigit():
            unusable += 1
            continue
        sent += int(s_cell.strip())
        replied += int(r_cell.strip()) if r_cell.strip().lstrip("-").isdigit() else 0

    valid = len(rows) - len(retracted) - unusable
    note = f" [rows={len(rows)} retracted={len(retracted)} unusable={unusable}]"
    if valid == 0:
        return None, (f"{ABSTAIN}: {path} holds no valid rows -- unmeasured, "
                      f"not zero.{note}")
    return f"{sent}/{replied}{note}", None
