...
# Structural references (rule ids, fix ids, phase/step ordinals, versions) are
# labels, not claims about the world, so they are exempt from the R2 tag sweep.
NUM_EXEMPT_CTX = re.compile(
    r"(phase\s*-?\d+|\bR\d\b|\bC\d+\b|\bFIX-?\d+\b|\bv\d(\.\d)?\b|step\s*\d+"
    r"|-{1,2}[a-zA-Z]*[= ]?\d+)",  # CLI flag args: tail -2, --maxdepth 6, -n 100
    re.I,
)
...
CITATION_PAT = re.compile(
    ...
    r"|^\./\S+|\s\./\S+"                  # relative script invocation: ./g gate
    r"|\b(ls|rg|grep|find|cat|wc|python3?|py|pytest|git|make|npm|jq|sed|awk|curl"
    r"|head|tail|tar|rm|mv|cp|chmod|mkdir|printf|echo|bash|sh|diff|sha256sum)\b)"
)
